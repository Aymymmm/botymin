...
def find_email_from_leaks(number):
    # نحاكي البحث في تسريبات معروفة على الانترنت
...
def handle_message(message):
    ...
    if not is_valid_yemeni_number(number):
        bot.send_message(message.chat.id, "❌ رقم غير صحيح. أرسل رقمًا يمنيًا صحيحًا فقط (73 / 71 / 70 / 77).")
        return
    ...
    report = f"""🛰️ Yemen BlackBox V3 🛰️
...
البريد المرتبط (إن وُجد من التسريبات): {leaked_email if leaked_email else "❌ لم يتم العثور عليه"}

- صنع بعقلية استخباراتية يمنية BlackBox V3
"""
