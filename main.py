...
def is_valid_yemeni_number(number):
    return bool(re.match(r'^(70|71|73|77)\d{7}$', number))

@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id, "أرسل رقم الهاتف اليمني بالصيغة: 73xxxxxxx (MTN) أو 71xxxxxxx (سبأفون) أو 70xxxxxxx (يو) أو 77xxxxxxx (يمن موبايل):")
...
def handle_message(message):
    ...
    if not is_valid_yemeni_number(number):
        bot.send_message(
            message.chat.id,
            "❌ رقم غير صحيح. يجب أن يكون رقم يمني صحيح مكون من 9 أرقام ويبدأ بـ:\n- 70 (شريحة يو)\n- 71 (شريحة سبأفون)\n- 73 (شريحة MTN)\n- 77 (شريحة يمن موبايل)"
        )
        return
    ...
    report = f"""🛰️ Yemen BlackBox V5 🛰️
...
- صنع بعقلية استخباراتية يمنية - BlackBox V5
"""
